...
BOOST_AUTO_TEST_SUITE(test_heuman_labda_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_heuman_labda, T, multiprecision_float_types)
{
    RandomSample<T> rng{-10, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::heuman_labda(x_ad,x_ad);
    auto y_expect = boost::math::heuman_labda(x,x);
