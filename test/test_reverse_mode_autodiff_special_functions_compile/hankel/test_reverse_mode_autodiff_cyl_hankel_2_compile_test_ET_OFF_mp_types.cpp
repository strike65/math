...
BOOST_AUTO_TEST_SUITE(test_cyl_hankel_2_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_cyl_hankel_2, T, multiprecision_float_types)
{
    RandomSample<T> rng{-10, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::cyl_hankel_2(x_ad,x_ad);
    auto y_expect = boost::math::cyl_hankel_2(x,x);
