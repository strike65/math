...
BOOST_AUTO_TEST_SUITE(test_hypergeometric_2F0_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_hypergeometric_2F0, T, bmp::cpp_bin_float_50)
{
    RandomSample<T> rng{-10, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::hypergeometric_2F0(x_ad,arg,arg);
    auto y_expect = boost::math::hypergeometric_2F0(x,arg,arg);
