...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_hypergeometric_1F0, T, multiprecision_float_types)
{
    RandomSample<T> rng{-1, 1};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::hypergeometric_1F0(x_ad,x_ad);
    auto y_expect = boost::math::hypergeometric_1F0(x,x);
