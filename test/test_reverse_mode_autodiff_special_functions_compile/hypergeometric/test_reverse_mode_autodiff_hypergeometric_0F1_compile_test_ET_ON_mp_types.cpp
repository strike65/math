...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_hypergeometric_0F1, T, bmp::cpp_bin_float_50)
{
    RandomSample<T> rng{-10, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::hypergeometric_0F1(x_ad,arg);
    auto y_expect = boost::math::hypergeometric_0F1(x,arg);
