...
BOOST_AUTO_TEST_SUITE(test_hypergeometric_0F1_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_hypergeometric_0F1, T, multiprecision_float_types)
{
    RandomSample<T> rng{-1, 1};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::hypergeometric_0F1(1,x_ad);
    auto y_expect = boost::math::hypergeometric_0F1(1,x);
