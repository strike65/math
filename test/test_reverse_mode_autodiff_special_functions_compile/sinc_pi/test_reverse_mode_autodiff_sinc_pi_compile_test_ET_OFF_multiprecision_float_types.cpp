    ...
    BOOST_AUTO_TEST_SUITE(test_sinc_pi_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_sinc_pi, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::sinc_pi(x_ad);
        auto y_expect = boost::math::sinc_pi(x);
