...
BOOST_AUTO_TEST_SUITE(test_spherical_harmonic_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_spherical_harmonic, T, multiprecision_float_types)
{
    RandomSample<T> rng{-10, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::spherical_harmonic(3,3,x_ad,x_ad);
    auto y_expect = boost::math::spherical_harmonic(3,3,x,x);
