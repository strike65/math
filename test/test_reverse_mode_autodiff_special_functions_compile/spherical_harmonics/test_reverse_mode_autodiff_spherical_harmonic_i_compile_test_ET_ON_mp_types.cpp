...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_spherical_harmonic_i, T, bmp::cpp_bin_float_50)
{
    RandomSample<T> rng{-10, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::spherical_harmonic_i(2,5,x_ad,arg);
    auto y_expect = boost::math::spherical_harmonic_i(2,5,x,arg);
