...
BOOST_AUTO_TEST_SUITE(test_rising_factorial_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_rising_factorial, T, multiprecision_float_types)
{
    RandomSample<T> rng{-10, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::rising_factorial(x_ad,3);
    auto y_expect = boost::math::rising_factorial(x,3);
