    ...
    BOOST_AUTO_TEST_SUITE(test_gegenbauer_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_gegenbauer, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::gegenbauer(2,1,x_ad);
        auto y_expect = boost::math::gegenbauer(2,1,x);
