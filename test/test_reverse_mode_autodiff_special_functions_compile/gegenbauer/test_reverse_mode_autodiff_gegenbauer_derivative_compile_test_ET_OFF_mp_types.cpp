...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_gegenbauer_derivative, T, bmp::cpp_bin_float_50)
{
    RandomSample<T> rng{-10, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::gegenbauer_derivative(3,x_ad,arg,2);
    auto y_expect = boost::math::gegenbauer_derivative(3,x,arg,2);
