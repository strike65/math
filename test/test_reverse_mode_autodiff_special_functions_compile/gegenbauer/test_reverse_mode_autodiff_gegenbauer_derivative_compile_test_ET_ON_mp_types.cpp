...
BOOST_AUTO_TEST_SUITE(test_gegenbauer_derivative_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_gegenbauer_derivative, T, multiprecision_float_types)
{
    RandomSample<T> rng{-10, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::gegenbauer_derivative(3,1,x_ad,2);
    auto y_expect = boost::math::gegenbauer_derivative(3,1,x,2);
