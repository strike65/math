...
BOOST_AUTO_TEST_SUITE(test_lambert_wm1_prime_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_lambert_wm1_prime, T, multiprecision_float_types)
{
    RandomSample<T> rng{-0.3, -0.1};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::lambert_wm1_prime(x_ad);
    auto y_expect = boost::math::lambert_wm1_prime(x);
