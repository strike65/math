...
BOOST_AUTO_TEST_SUITE(test_legendre_q_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_legendre_q, T, multiprecision_float_types)
{
    RandomSample<T> rng{-1, 1};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::legendre_q(3,x_ad);
    auto y_expect = boost::math::legendre_q(3,x);
