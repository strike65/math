    ...
    BOOST_AUTO_TEST_SUITE(test_laguerre_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_laguerre, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::laguerre(1,3,x_ad);
        auto y_expect = boost::math::laguerre(1,3,x);
