    ...
    BOOST_AUTO_TEST_SUITE(test_ibeta_invb_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_ibeta_invb, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::ibeta_invb(x_ad,x_ad,x_ad);
        auto y_expect = boost::math::ibeta_invb(x,x,x);
