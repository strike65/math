    ...
    BOOST_AUTO_TEST_SUITE(test_ibetac_invb_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_ibetac_invb, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::ibetac_invb(x_ad,x_ad,x_ad);
        auto y_expect = boost::math::ibetac_invb(x,x,x);
