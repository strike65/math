...
BOOST_AUTO_TEST_SUITE(test_sqrt1pm1_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_sqrt1pm1, T, multiprecision_float_types)
{
    RandomSample<T> rng{5, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::sqrt1pm1(x_ad);
    auto y_expect = boost::math::sqrt1pm1(x);
