    ...
    BOOST_AUTO_TEST_SUITE(test_hypot_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_hypot, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::hypot(x_ad,x_ad);
        auto y_expect = boost::math::hypot(x,x);
