    ...
    BOOST_AUTO_TEST_SUITE(test_log1p_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_log1p, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::log1p(x_ad);
        auto y_expect = boost::math::log1p(x);
