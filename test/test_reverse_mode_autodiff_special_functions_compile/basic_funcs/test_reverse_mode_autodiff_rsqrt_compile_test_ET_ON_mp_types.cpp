...
BOOST_AUTO_TEST_SUITE(test_rsqrt_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_rsqrt, T, multiprecision_float_types)
{
    RandomSample<T> rng{1, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::rsqrt(x_ad);
    auto y_expect = boost::math::rsqrt(x);
