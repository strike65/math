...
BOOST_AUTO_TEST_SUITE(test_polygamma_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_polygamma, T, multiprecision_float_types)
{
    RandomSample<T> rng{-5, 5};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::polygamma(1,x_ad);
    auto y_expect = boost::math::polygamma(1,x);
