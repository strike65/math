...
BOOST_AUTO_TEST_SUITE(test_trigamma_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_trigamma, T, multiprecision_float_types)
{
    RandomSample<T> rng{-5, 5};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::trigamma(x_ad);
    auto y_expect = boost::math::trigamma(x);
