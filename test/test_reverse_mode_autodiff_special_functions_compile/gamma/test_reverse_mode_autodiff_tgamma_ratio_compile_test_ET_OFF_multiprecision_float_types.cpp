    ...
    BOOST_AUTO_TEST_SUITE(test_tgamma_ratio_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_tgamma_ratio, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::tgamma_ratio(x_ad,x_ad);
        auto y_expect = boost::math::tgamma_ratio(x,x);
