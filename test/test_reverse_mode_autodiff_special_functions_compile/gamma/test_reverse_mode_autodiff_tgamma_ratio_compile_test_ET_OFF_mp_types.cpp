...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_tgamma_ratio, T, bmp::cpp_bin_float_50)
{
    RandomSample<T> rng{0, 5};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::tgamma_ratio(x_ad,arg);
    auto y_expect = boost::math::tgamma_ratio(x,arg);
