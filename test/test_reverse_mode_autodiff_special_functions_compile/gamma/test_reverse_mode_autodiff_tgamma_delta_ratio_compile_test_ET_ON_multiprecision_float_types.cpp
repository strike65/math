    ...
    BOOST_AUTO_TEST_SUITE(test_tgamma_delta_ratio_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_tgamma_delta_ratio, T, multiprecision_float_types)
    {
        RandomSample<T> rng{ 0, 5
 };
        T               x = rng.next();
    
        rvar<T, 1> x_ad = x;
        auto y = boost::math::tgamma_delta_ratio(x_ad,x_ad);
        auto y_expect = boost::math::tgamma_delta_ratio(x,x);
