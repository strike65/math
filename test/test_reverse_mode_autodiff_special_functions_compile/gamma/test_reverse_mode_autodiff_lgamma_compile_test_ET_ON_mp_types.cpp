...
BOOST_AUTO_TEST_SUITE(test_lgamma_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_lgamma, T, multiprecision_float_types)
{
    RandomSample<T> rng{-10, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::lgamma(x_ad);
    auto y_expect = boost::math::lgamma(x);
