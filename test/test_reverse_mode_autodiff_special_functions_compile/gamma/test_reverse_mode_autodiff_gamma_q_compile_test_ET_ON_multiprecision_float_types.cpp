    ...
    BOOST_AUTO_TEST_SUITE(test_gamma_q_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_gamma_q, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::gamma_q(x_ad,x_ad);
        auto y_expect = boost::math::gamma_q(x,x);
