    ...
    BOOST_AUTO_TEST_SUITE(test_jacobi_double_prime_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_jacobi_double_prime, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::jacobi_double_prime(2,x_ad,x_ad,x_ad);
        auto y_expect = boost::math::jacobi_double_prime(2,x,x,x);
