    ...
    BOOST_AUTO_TEST_SUITE(test_jacobi_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_jacobi, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::jacobi(2,x_ad,x_ad,x_ad);
        auto y_expect = boost::math::jacobi(2,x,x,x);
