...
BOOST_AUTO_TEST_SUITE(test_jacobi_sn_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_jacobi_sn, T, multiprecision_float_types)
{
    RandomSample<T> rng{-10, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::jacobi_sn(0.25,x_ad);
    auto y_expect = boost::math::jacobi_sn(0.25,x);
