    ...
    BOOST_AUTO_TEST_SUITE(test_jacobi_ds_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_jacobi_ds, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::jacobi_ds(0.25,x_ad);
        auto y_expect = boost::math::jacobi_ds(0.25,x);
