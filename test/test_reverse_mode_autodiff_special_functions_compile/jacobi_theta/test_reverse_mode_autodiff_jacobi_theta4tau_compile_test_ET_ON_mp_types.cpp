...
BOOST_AUTO_TEST_SUITE(test_jacobi_theta4tau_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_jacobi_theta4tau, T, multiprecision_float_types)
{
    RandomSample<T> rng{-10, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::jacobi_theta4tau(x_ad,0.5);
    auto y_expect = boost::math::jacobi_theta4tau(x,0.5);
