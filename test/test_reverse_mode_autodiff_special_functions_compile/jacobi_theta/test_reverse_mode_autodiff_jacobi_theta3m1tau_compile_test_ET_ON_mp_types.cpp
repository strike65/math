...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_jacobi_theta3m1tau, T, bmp::cpp_bin_float_50)
{
    RandomSample<T> rng{-10, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::jacobi_theta3m1tau(x_ad,arg);
    auto y_expect = boost::math::jacobi_theta3m1tau(x,arg);
