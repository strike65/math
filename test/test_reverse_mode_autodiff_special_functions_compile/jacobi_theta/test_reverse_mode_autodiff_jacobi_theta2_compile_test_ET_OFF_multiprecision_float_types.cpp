    ...
    BOOST_AUTO_TEST_SUITE(test_jacobi_theta2_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_jacobi_theta2, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::jacobi_theta2(x_ad,0.5);
        auto y_expect = boost::math::jacobi_theta2(x,0.5);
