    ...
    BOOST_AUTO_TEST_SUITE(test_cyl_bessel_k_prime_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_cyl_bessel_k_prime, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::cyl_bessel_k_prime(x_ad,x_ad);
        auto y_expect = boost::math::cyl_bessel_k_prime(x,x);
