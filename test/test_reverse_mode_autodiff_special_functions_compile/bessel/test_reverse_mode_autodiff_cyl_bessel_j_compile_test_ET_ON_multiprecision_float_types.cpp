    ...
    BOOST_AUTO_TEST_SUITE(test_cyl_bessel_j_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_cyl_bessel_j, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::cyl_bessel_j(0.25,x_ad);
        auto y_expect = boost::math::cyl_bessel_j(0.25,x);
