...
BOOST_AUTO_TEST_SUITE(test_cyl_neumann_prime_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_cyl_neumann_prime, T, multiprecision_float_types)
{
    RandomSample<T> rng{0, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::cyl_neumann_prime(x_ad,x_ad);
    auto y_expect = boost::math::cyl_neumann_prime(x,x);
