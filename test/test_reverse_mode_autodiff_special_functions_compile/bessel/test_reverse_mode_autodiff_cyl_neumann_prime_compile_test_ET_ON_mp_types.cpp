...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_cyl_neumann_prime, T, bmp::cpp_bin_float_50)
{
    RandomSample<T> rng{0, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::cyl_neumann_prime(x_ad,arg);
    auto y_expect = boost::math::cyl_neumann_prime(x,arg);
