...
BOOST_AUTO_TEST_SUITE(test_sph_neumann_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_sph_neumann, T, multiprecision_float_types)
{
    RandomSample<T> rng{0, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::sph_neumann(1,x_ad);
    auto y_expect = boost::math::sph_neumann(1,x);
