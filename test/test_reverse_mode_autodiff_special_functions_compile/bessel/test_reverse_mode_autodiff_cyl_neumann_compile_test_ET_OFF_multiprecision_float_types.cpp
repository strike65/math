    ...
    BOOST_AUTO_TEST_SUITE(test_cyl_neumann_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_cyl_neumann, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::cyl_neumann(x_ad,x_ad);
        auto y_expect = boost::math::cyl_neumann(x,x);
