    ...
    BOOST_AUTO_TEST_SUITE(test_sph_bessel_prime_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_sph_bessel_prime, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::sph_bessel_prime(x_ad,x_ad);
        auto y_expect = boost::math::sph_bessel_prime(x,x);
