    ...
    BOOST_AUTO_TEST_SUITE(test_ellint_rj_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_ellint_rj, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::ellint_rj(x_ad,x_ad,x_ad,x_ad);
        auto y_expect = boost::math::ellint_rj(x,x,x,x);
