...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_ellint_rd, T, bmp::cpp_bin_float_50)
{
    RandomSample<T> rng{-10, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::ellint_rd(x_ad,arg,arg);
    auto y_expect = boost::math::ellint_rd(x,arg,arg);
