...
BOOST_AUTO_TEST_SUITE(test_ellint_rd_compiles)

using namespace rdiff;

BOOST_AUTO_TEST_CASE_TEMPLATE(test_ellint_rd, T, multiprecision_float_types)
{
    RandomSample<T> rng{0, 10};
    T               x = rng.next();

    rvar<T, 1> x_ad = x;
    auto y = boost::math::ellint_rd(x_ad,x_ad,x_ad);
    auto y_expect = boost::math::ellint_rd(x,x,x);
