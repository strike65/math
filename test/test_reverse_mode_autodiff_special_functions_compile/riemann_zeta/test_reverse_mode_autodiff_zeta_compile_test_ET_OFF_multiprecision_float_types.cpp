    ...
    BOOST_AUTO_TEST_SUITE(test_zeta_compiles)
    
    using namespace rdiff;
    
    BOOST_AUTO_TEST_CASE_TEMPLATE(test_zeta, T, multiprecision_float_types)
    {
        ...
        rvar<T, 1> x_ad = x;
        auto y = boost::math::zeta(x_ad);
        auto y_expect = boost::math::zeta(x);
